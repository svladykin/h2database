...
public class ConditionInConstantSet extends Condition {
    ...
    public ConditionInConstantSet(final Session session, Expression left,
        ...
        for (Expression expression : valueList) {
            valueSet.add(expression.getValue(session).convertTo(type, session.getDatabase()));
        }
    }
    ...
    Expression getAdditional(Session session, Comparison other) {
        ...
        if (add != null) {
            if (add.isConstant()) {
                valueList.add(add);
                valueSet.add(add.getValue(session).convertTo(left.getType(), session.getDatabase()));
                return this;
            }
        }
        return null;
