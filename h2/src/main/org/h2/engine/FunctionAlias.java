    ...
    public static class JavaMethod implements Comparable<JavaMethod> {
        ...
        public Value getValue(Session session, Expression[] args,
            ...
            for (int a = 0, len = args.length; a < len; a++, p++) {
                ...
                if (Value.class.isAssignableFrom(paramClass)) {
                    ...
                    for (int i = 0; i < objArray.length; i++) {
                        objArray[i] = array[i].convertTo(componentType, session.getDatabase()).getObject();
                    }
                    o = objArray;
                } else {
                    v = v.convertTo(type, session.getDatabase());
                    o = v.getObject();
                }
                if (o == null) {
            ...
            try {
                ...
                return ret.convertTo(dataType, session.getDatabase());
            } finally {
